...
export const collectionsWithExtraData = (collections) => {
  return collections.map((c) => {
    ...
    let storagePathIdentifier = c.collectionData && c.collectionData.storagePath.identifier
    let publicPathIdentifier = c.collectionData && c.collectionData.publicPath.identifier
...
export const collectionsWithCatalogInfo = (collections, nftCatalog) => {
  ...
  if (Object.keys(nftCatalog).length == 0) {
    nftCatalog = []
  }

  for (const catalog of nftCatalog) {
    const path = catalog.path.storage
    catalogPathMap[path] = {
      catalog: catalog
    }
  }
  ...
  for (let i = 0; i < collections.length; i++) {
    ...
    if (catalogInfo) {
      collection.socials = {
        twitter: {url: catalogInfo.catalog.extensions.twitter},
        discord: {url: catalogInfo.catalog.extensions.discord},
        instagram: {url: catalogInfo.catalog.extensions.instagram}
      }
      collection.externalURL = {url: catalogInfo.catalog.extensions.website}
      // collection.description = catalogInfo.catalog.description
      // collection.contractName = catalogInfo.catalog.contractName
      // collection.contractAddress = catalogInfo.catalog.address
      collection.uuid = `A.${collection.address.replace("0x", "")}.${collection.contractName}`
    } else {
      if (collection.contract) {
        const elements = collection.contract.split(".")
        if (elements.length == 3) {
          collection.contractAddress = `0x${elements[1]}`
          collection.contractName = elements[2]
          collection.uuid = collection.contract
        }
      }
    }
...
export const flowmapToName = (inscription) => {
  if (inscription == "") {
    return "Unknown"
  }
  let flowmapNumber = parseInt(inscription.replace(".flowmap", ""))
  const chars = 'abcdefghijklmnopqrstuvwxyz';
  const base = chars.length;
  let name = '';

  while (flowmapNumber > 0) {
      const remainder = flowmapNumber % base;
      name += chars[remainder];
      flowmapNumber = Math.floor(flowmapNumber / base);
  }

  return name.split('').reverse().join('');
}

export const flowmapDisplayName = (inscription) => {
  return `${inscription}\nname: ${flowmapToName(inscription)}`
}
