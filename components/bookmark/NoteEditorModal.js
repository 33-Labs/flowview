...
import { showNoteEditorState, accountBookmarkState, transactionStatusState, transactionInProgressState } from '../../lib/atoms'
import { addOrEditAccountBookmark } from '../../flow/bookmark_transactions'
...
export default function NoteEditorModal(props) {
  ...
  const [newNote, setNewNote] = useState(accountBookmark.note)
  ...
  return (
    <Transition.Root show={showNoteEditor} as={Fragment}>
      <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRef} onClose={setShowNoteEditor}>
        <Transition.Child
          as={Fragment}
                ...
                <div>
                  <div className="mt-3">
                    ...
                    <input
                     ...
                     defaultValue={accountBookmark.note}
                      onChange={(event) => {
                ...
                <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      await addOrEditAccountBookmark(accountBookmark.address, newNote, setTransactionInProgress, setTransactionStatus)
                      if (user && user.loggedIn) {
                        mutate(["accountBookmarksFetcher", user.addr])
                        mutate(["accountBookmarkFetcher", user.addr, accountBookmark.address])
                      }
                    }}
                  >
                    {"Confirm"}
